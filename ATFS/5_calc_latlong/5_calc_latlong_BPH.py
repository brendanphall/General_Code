...
with open(sqlfile, "w") as f_sql:
	...
	with arcpy.da.SearchCursor(shapefile_path, ['treefarm_i', 'totalfores', 'longitude', 'latitude', 'totalacres']) as cursor:
		for row in cursor:
			...
			sql1 = "update treefarm set totalacres = {0}, totalfores = {1} where treefarm_i = {2}".format(totalacres, totalfores, treefarm_i)
			sql2 = "update treefarmlocation set acres = {0}, longitude = '{1}', latitude = '{2}', forestedacres = {3} where treefarm_i = {4}".format(totalacres, longitude, latitude, totalfores, treefarm_i)
